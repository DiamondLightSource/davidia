...
export class AxisSelection extends BaseSelection {
...
/** export class for a horizontal axis selection */
export class HorizontalAxisSelection extends AxisSelection {
  readonly horizontalAxis: boolean = true;
  ...
  static isShape(
    s: HorizontalAxisSelection | AxisSelection | SelectionBase
  ): s is HorizontalAxisSelection {
    return 'horizontalAxis' in s;
  }
  static createFromPoints(points: Vector3[]) {
    const orderedPoints =
    ...
    const r = new HorizontalAxisSelection(
  ...
  static createFromSelection(s: HorizontalAxisSelection) {
    const r = new HorizontalAxisSelection([...s.start], [...s.dimensionLength]);
    r.setProperties(s.id, s.name, s.colour, s.alpha);
    r.setFixed(s.fixed);
    return r;
  }
  ...
  onHandleChange(i: number, pos: [number | undefined, number | undefined]) {
    const r = HorizontalAxisSelection.createFromSelection(this);
    const o = this.getPoint(i);
    if (o !== null) {
      const x = pos[0] ?? 0;
      const y = pos[1] ?? 0;
...
export class VerticalAxisSelection extends AxisSelection {
